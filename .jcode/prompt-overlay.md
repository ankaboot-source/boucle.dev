...
## Doc maintenance — update in the same PR

After implementing, check whether your changes require doc updates. **Doc updates go in the same commit/PR as the code change — never a separate PR.**
...
4. After implementing, copy the Approach and Scope Self-Check from PLAN.md into `state.md` (the PR description reads from state.md's Approach section).
...
4. **Read the "Prior feedback on the PR" section of your prompt** (if present). It contains reviewer verdicts (`VERDICT: FAIL` with the unmet acceptance criteria) and human comments on the PR. You MUST address every actionable item before claiming done — a re-run that ignores prior feedback will FAIL the reviewer the same way again and waste the iteration budget. Map each unmet criterion to a concrete change in your implementation.
...
6. **Issue attachments** (paths listed in your prompt under "Issue attachments") may be source assets to ship (logos, photos, visuals) OR mockups/screenshots for context. Decide based on the issue body and comment intent. For each file:
 ...
 7. **PR comment attachments** (paths listed in your prompt under "PR comment attachments") have the same dual nature — mockups/screenshots for context OR source assets to ship. Decide based on the comment intent:
  ...
  12. Update `state.md`:
    ...
    - **If human PR comments amended the spec** (new or changed requirements vs the triage-era criteria), update the `## Acceptance criteria` section of `state.md` to reflect the amended spec — mark each amended criterion with `(amended via PR comment)`. `state.md` is seeded once from the triage comment and never refreshed automatically; without this update the criteria drift from what the human actually asked for, and the reviewer grades against a stale spec.
    - **Record spec amendments as deltas.** When a human PR comment amends the spec, also record the amendment in the `## Spec delta` section of `state.md` using the delta format:
      ```
      - **ADDED** <criterion> — (source: PR comment by <author>, <date>)
      - **MODIFIED** <old criterion> → <new criterion> — (source: PR comment by <author>, <date>)
      - **REMOVED** <criterion> — (source: PR comment by <author>, <date>)
      ```
  ...
  14. **Update charter docs** if your changes impact them (see "Doc maintenance" above). Commit doc updates in the same PR as the code.
...
**Surface, don't silently expand.** If you believe extra work is needed (a refactor, a missing sibling feature, a related bug), do NOT add it to the PR silently. Record it in `state.md` under "Awaiting human" or as a follow-up note. The reviewer grades the PR against the spec, and an unrequested change is a FAIL criterion (scope creep) — even a good one.
...
- **Do NOT** rebase or merge master into your branch — the job rebases onto master after you commit. If you rebase yourself, you risk losing `MERGE_HEAD` and producing a single-parent commit that leaves the PR conflicted.
